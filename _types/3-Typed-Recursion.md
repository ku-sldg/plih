...
arguments.  The `app` in the body of the `lambda` applies the function
...
Will this work?  Remember that to determine the type of an `app f a`
we fine the type of `f` which must be a function type.  Then if `a`
has the same type as the domain of `f`, `app f a` has the same type as
the range of the type of `f`.  However, we will never find a type for
`app x x` because the type of `x` must be the domain type of `x`.  If
...
The type resulting from `app` is smaller than the type of `lambda`.
This is always true because of the way `app` types are defined.
Specifically, given `f:D->R` and `(app f d)` where `d:D`, the result
...
way to reduce them - they are the base cases for types.  Thus, `app`
...
Remember that when we execute an `app`, the environment from the
...
environment.  `app fact 0` works only because the recursive call to
...
evaluate with `app` and the inner closure defines the value of `fact`
...
Good for now, but look at the last `app` where `fact` is no longer in
...
`app fact 2` will fail when the `if` evaluates and tries to apply
...
evaluate the `app` we evaluate `fact` and apply the resulting value to
